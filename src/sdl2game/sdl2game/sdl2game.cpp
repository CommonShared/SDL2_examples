// sdl2demo.cpp : Defines the entry point for the application.
//
...
void ltexture::load(const char* path) {
...
public:
	...
	int curtexture = 0;
	button() {}
	void load(const char* path, int i = 0);
...
void button::load(const char* path, int i) {
	tex[i].load(path);
...
void button::handleevent(SDL_Event e) {
	...
	if (e.motion.x * (W1080 / WindowW) < x || 
		e.motion.x * (W1080 / WindowW) > x + w || 
		e.motion.y * (H1080 / WindowH) < y || 
		e.motion.y * (H1080 / WindowH) > y + h) {
...
void scoreboard::load(const char* path) {
	texture.load(path);
...
int cal(void* ptr) {
	pass.enabled = false;
	back.enabled = false;
	message = (char*)"Thinking...";
	test.cvc();
	test.getarray(qipan.q);
	pass.enabled = true;
	back.enabled = true;
	message = (char*)"Your turn";
	return 0;
}
//init
void init() {
...
void buttonevent() {
	if (start.isclicked) {
		...
		int r = rand() % (2);
		if (r == 1) { test.cvc(); message = (char*)"Back hand"; }
		else message = (char*)"First hand";
		test.getarray(qipan.q);
		scene = 1;
