...
int qp::input(int x, int y){
	if (xsum == 0){
		message = (char*)"      white win";
		return 100;
	}
	if (osum == 0){
		message = (char*)"      black win";
		return 100;
	}
	if (xsum + osum == xm*ym) {
		message = (char*)"      the game is over";
		return -100;
	}
	if (x>xm || x<1 || y>ym || y<1){
		message = (char*)"      input err";
		return -1;
	}
	if (q[x - 1][y - 1] != 0){
		message = (char*)"      overlaped";
		return -1;
	}
	struct board temp(q, available,flag);
	point p(x - 1, y - 1);
...
int qp::shengfu(){
	if ((xsum + osum == xm*ym)||(passsum==2)){
		if (xsum > osum){ message = (char*)"     black win"; return 1; }
		else if (xsum < osum){ message = (char*)"     white win";return 2; }
		else if (xsum == osum){ message = (char*)"      pingju"; return 3; }
...
void qp::print(){
	...
	message = (char*)"";
	char *turns;
	(flag==1)?turns= (char*)"blank":turns= (char*)"white";
